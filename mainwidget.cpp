...
{
    //inicialila a imagem passando os paramentros
    ...
    //mosta as linhas preta existente na tela
    for (int j = -15; j < 25; ++j)
    {
...
void pixelLarguraAltura(QImage* target, int x, int y)
{
    /*aqui irá verificar a altura e largura que deverá ser preenchido
      pela cor vermelha */
    if (x >= 0
        && y >=0
        && x < target->width()
...
{
    *buffer = *img;
    qDebug() << "inicio";//apenas verifica se não tem bugs
    //aqui começa a pintar a janela
    for (int y = 0; y < 50; ++y)
    {
        for (int x = 0; x < 50; ++x)
        {
            ...
            {
                pixelLarguraAltura(buffer, x, y+1);
                pixelLarguraAltura(buffer, x, y-1);
                pixelLarguraAltura(buffer, x+1, y);
                pixelLarguraAltura(buffer, x-1, y);
